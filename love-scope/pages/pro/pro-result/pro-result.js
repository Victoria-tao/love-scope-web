// pages/pro/pro-result/pro-result.js 专业版深度匹配综合结果
...
Page({
  ...
  buildSummary(r, score, level) {
    ...
    if (r.birthMatch) {
      parts.push('出生年柱' + r.birthMatch.stemRel.type + '，' + r.birthMatch.ageText);
    }
    if (r.synastry) {
  ...
  _drawShareCard() {
    ...
    query.select('#shareCanvas').fields({ node: true, size: true }).exec((res) => {
      ...
      if (hasTarot) {
        ...
        ctx.fillText('🎴 趣味卡牌指引：' + this.data.tarot.card.name + '（' + (this.data.tarot.upright ? '正位' : '逆位') + '）', W / 2, curY);
